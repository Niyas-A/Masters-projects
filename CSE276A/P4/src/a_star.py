...
# def wp():
def get_wp(scaling=4.0, heuristic="distance"):
    grid_size = 11
    ...
    for i in range(grid_size):
        grid2[i][0] = 1
        grid2[0][i] = 1
        grid2[grid_size - 1][i] = 1
        grid2[i][grid_size - 1] = 1

    for i in range(4, 6):
        for j in range(4, 6):
            grid2[i][j] = 1
    path = a_star(grid2, start_point, goal_point, hueristic_name=heuristic)
    starting_point = path[0]
    points = [(y - starting_point[1], x - starting_point[0]) for x, y in path]
    waypoint = []
    prev_x, prev_y = points[0][0], points[0][1]
    for x, y in points:
        angle = smart_atan((y - prev_y), (x - prev_x))
        waypoint.append([x / scaling, y / scaling, angle])
        prev_x = x
        prev_y = y
    waypoint = reduce_wp(waypoint)
