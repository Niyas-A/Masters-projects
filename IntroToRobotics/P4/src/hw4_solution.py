...
if __name__ == "__main__":
    ...
    # NOTE: comment out the line in order to run distnace vs safety. Safety works much better with our calibration
    # waypoint = get_wp(scaling=12.0, heuristic="distance")
    waypoint = get_wp(scaling=12.0, heuristic="safety")

    # init pid controller
    pid = PIDcontroller(0.15, 0.0015, 0.004)
    ...
    for wp in waypoint:
        ...
        while(np.linalg.norm(pid.getError(current_state, wp)) > 0.1): # check the error between current state and current way point
            ...
            if found_state:
                current_state = estimated_state
                print("Estimated state", current_state)
