...
def main():
    ...
    cur_iter = 0
    x_range = np.linspace(-3, 3, 5)
    y_range = np.linspace(-3, 3, 5)
    theta_range = np.linspace(-np.pi, np.pi, 5)
    ...
    while cur_iter * utils.time_step < utils.sim_time:
        ...
        xi = find_index(cur_err[0], x_range)
        yi = find_index(cur_err[1], y_range)
        thetai = find_index(cur_err[2], theta_range)
