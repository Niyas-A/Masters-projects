...
for i in range(0,500):
    ...
    if i%10 == 0:
        ...
        log_odds_map1 = 1/exp_log_odds
        plt.figure(figsize=(10, 6))


        plt.imshow(log_odds_map1.T, cmap='binary', origin='lower')
        imu_plot_x = np.ceil((pose_imu[0,0:i] - MAP['xmin']) / MAP['res']).astype(np.int16) - 1
        imu_plot_y = np.ceil((pose_imu[1, 0:i] - MAP['xmin']) / MAP['res']).astype(np.int16) - 1
        plt.plot(imu_plot_x, imu_plot_y, color='red', linewidth=2)
        plt.xlabel('X - grid')
        plt.ylabel('Y - grid')
...
# frame_folder = 'frames'
# output_video = 'occupancy_map_video.mp4'
