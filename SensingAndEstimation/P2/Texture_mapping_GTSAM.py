...
plt.figure()
plt.imshow(map_rgb)
gtsam_pose_x = np.ceil((gtsam_pose[1, :] - MAP['xmin']) / MAP['res']).astype(np.int16) - 1
gtsam_pose_y = np.ceil((gtsam_pose[0, :] - MAP['ymin']) / MAP['res']).astype(np.int16) - 1
plt.plot(gtsam_pose_x, gtsam_pose_y, color='red', linewidth=2)
plt.xlabel('Y - grid')
plt.ylabel('X - grid')
plt.title('Texture Map with GTSAM trajectory')
plt.show()